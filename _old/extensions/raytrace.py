...
def exponential_integration(feats, tau, feats_t, tau_t, boundaries, exclusive=True):
    ...
    alpha_t = 1.0 - torch.exp(-tau_t.contiguous())
    
    # TRANSMITTANCE
    tau_tot = tau + tau_t
    transmittance = torch.exp(-1.0 * cumsum(tau_tot.contiguous(), boundaries.contiguous(), exclusive=exclusive))

    # Color from STATIC
    transmittance_static = transmittance * alpha

    # Color from TRANSIENT
    transmittance_trans = transmittance * alpha_t

    # Final color and transmittance
    feats_out = sum_reduce( transmittance_static * feats.contiguous() + transmittance_trans * feats_t.contiguous(), 
                            boundaries.contiguous())
    transmittance = transmittance_static + transmittance_trans
